...
class MLCourseGUI(QMainWindow):
    ...
    def init_layer_parameter_widgets(self):
        # Dense Layer
        dense_widget = QWidget()
        dense_layout = QVBoxLayout()
        self.dense_units = QSpinBox()
        self.dense_units.setRange(1, 10000)
        self.dense_activation = QComboBox()
        self.dense_activation.addItems(["relu", "sigmoid", "tanh"])
        dense_layout.addWidget(QLabel("Units:"))
        dense_layout.addWidget(self.dense_units)
        dense_layout.addWidget(QLabel("Activation:"))
        dense_layout.addWidget(self.dense_activation)
        dense_widget.setLayout(dense_layout)
        self.layer_params.addWidget(dense_widget)
        
        # Conv2D Layer
        conv_widget = QWidget()
        conv_layout = QVBoxLayout()
        self.conv_filters = QSpinBox()
        self.conv_kernel = QLineEdit("3,3")
        conv_layout.addWidget(QLabel("Filters:"))
        conv_layout.addWidget(self.conv_filters)
        conv_layout.addWidget(QLabel("Kernel Size:"))
        conv_layout.addWidget(self.conv_kernel)
        conv_widget.setLayout(conv_layout)
        self.layer_params.addWidget(conv_widget)
        c
    ...
    def create_tabs(self):
        ...
        tabs = [
            ...
            ("Advanced DL", self.create_advanced_dl_tab), 
            ("Dimensionality Reduction", self.create_dim_reduction_tab),
            ("Reinforcement Learning", self.create_rl_tab),
            ("Advanced Features", self.create_advanced_tab)
        ]
    ...
    def create_advanced_dl_tab(self):
        widget = QWidget()
        layout = QVBoxLayout(widget)
        ...
        self.layer_type_combo.addItems(["Dense", "Conv2D", "LSTM", "GRU", "Dropout"])
    
        # Layer Parameters Stack
        self.layer_params = QStackedWidget()
        self.init_layer_parameter_widgets()
    
        # Control Buttons
        btn_add = QPushButton("Add Layer", clicked=self.add_dl_layer)
        btn_remove = QPushButton("Remove Selected", clicked=self.remove_dl_layer)
        ...
        # Assemble UI
        arch_layout.addWidget(QLabel("Layer Type:"))
        arch_layout.addWidget(self.layer_type_combo)
        arch_layout.addWidget(self.layer_params)
        arch_layout.addWidget(btn_add)
        arch_layout.addWidget(btn_remove)
        arch_layout.addWidget(self.layer_list)
    
        train_btn = QPushButton("Train Model", clicked=self.train_advanced_model)
...
class DQNAgent:
    ...
    def add_dl_layer(self):
        layer_type = self.layer_type_combo.currentText()
        layer = {"type": layer_type}
        
        if layer_type == "Dense":
            layer.update({
                "units": self.dense_units.value(),
                "activation": self.dense_activation.currentText()
            })
        elif layer_type == "Conv2D":
            layer.update({
                "filters": self.conv_filters.value(),
                "kernel_size": tuple(map(int, self.conv_kernel.text().split(',')))
            })
        
        self.dl_layers.append(layer)
        self.layer_list.addItem(f"{layer_type} layer")

    def remove_dl_layer(self):
        selected = self.layer_list.currentRow()
        if selected >= 0:
            self.dl_layers.pop(selected)
            self.layer_list.takeItem(selected)

    def train_advanced_model(self):
        model = models.Sequential()
        for layer in self.dl_layers:
            if layer["type"] == "Dense":
                model.add(layers.Dense(**layer))
            elif layer["type"] == "Conv2D":
                model.add(layers.Conv2D(**layer))
        
        model.compile(optimizer=self.optimizer_combo.currentText(),
